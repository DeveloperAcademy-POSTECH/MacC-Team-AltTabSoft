...
public class Bullet : MonoBehaviour
{
    ...
    IEnumerator Goodbye()
    {
        if (type != Type.BulletExplosion && type != Type.Laser && type != Type.BombBlast)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.layer == LayerMask.NameToLayer("Monster") && type != Type.BombBlast)
        {
            ...
            if(type != Type.Laser && type != Type.BulletExplosion)
                trail.Clear();
            ...
            if(type == Type.Bomb && other != null && (other.transform.Find("BombOnMonster") == null || other.transform.Find("BombOnMonster").gameObject.activeSelf == false))
            {
                ...
                InstantBomb.transform.position = other.transform.position + new Vector3(0, 3f, 0);
            }
        }

        //총알이 맵 바닥에 닿으면 사라지는 경우
        if ((other.gameObject.layer == LayerMask.NameToLayer("Map_Object") || other.gameObject.layer == LayerMask.NameToLayer("Map_Floor")) && type != Type.Laser && type != Type.BombBlast && type != Type.BulletExplosion)
        {
            trail.Clear();
            ObjectPoolManager.Inst.DestroyObject(this.gameObject);
        }
